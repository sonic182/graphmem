...
- `update`: revises a memory by numeric `id`. `content`, `memory_type`, and
  `importance` replace the stored values; omitted fields are kept. Scopes,
...
`forget` apply the same scope rule recall does: the server reaches a memory
that has no scopes, a `global` memory, or one in its own repository scope, and
reports `memory not found` for anything else. A guessed id from another
repository therefore reveals nothing and changes nothing. The `gmem` CLI is the
